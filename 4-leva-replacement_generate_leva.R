...
tse_2022 = open_tse_file('./data/tse/tse_2022.csv') %>%
  rename(NR_IDENTIFICACAO_PESQUISA = NR_PROTOCOLO_REGISTRO, DS_CARGOS = DS_CARGO) %>%
...
read_csv('bing_input.txt') %>%
  mutate(tse_id = str_replace_all(tse_id, '"', '')) %>%
  left_join(tse_2022, c('tse_id' = 'f_id')) %>%
  mutate(tse_id = paste0('"', tse_id, '"')) %>%
  group_split(tse_id) %>%
  map_dfr(function(x) { add_row(x, tse_id='', SG_UF='', NM_UE='', DS_CARGOS='') }) %>%
  write.csv('leva_g_new.csv', row.names = F)
