...
model_polls = late_polls %>%
  ...
  summarize(mm3 = pop.sd(valid_result - pct), undecided = first(undecided),
            correct_result = order(pct, decreasing = T)[1] == 1,
            correct_second = order(pct, decreasing = T)[2] == 2) %>%
  ungroup() %>%
...
simple_rating = simple_rating_0 %>%
  ...
  mutate(pred_pm_alt = mean(weighted_pm) + (weighted_pm - mean(weighted_pm)) * (n / (n + 30))) %>%
  mutate_if(is.double, round, 3) %>%
  filter(n >= 5) %>%
  mutate(bin = .bincode(pred_pm, hist(pred_pm, plot = F)$breaks)) %>%
  mutate(grade = recode(bin, `1` = 'A', `2` = 'A', `3` = 'B+', `4` = 'B', `5` = 'B-', `6` = 'C', `7` = 'D'))
